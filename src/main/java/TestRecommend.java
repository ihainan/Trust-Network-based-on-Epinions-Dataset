...
public class TestRecommend {
    ...
    public static void main(String[] args) {
        ...
        Map<String, Double> simUserSimMap = new HashMap<String, Double>();  // 其他用户与用户 p5 的相似度 Map
        ...
        pobjMap3.put("环太平洋", 5);
        pobjMap3.put("变形金刚", 5);
        simUserObjMap.put("p3", pobjMap3);  // 根据 1，2，3 用户的喜好，为用户 5 推荐作品
    ...
    public static String getRecommend(Map<String, Map<String, Integer>> simUserObjMap, Map<String, Double> simUserSimMap) {
        ...
        for (Entry<String, Map<String, Integer>> simUserEn : simUserObjMap.entrySet()) {    // 对于每个用户的喜好
            String user = simUserEn.getKey();   // 对应的用户
            double sim = simUserSimMap.get(user);   // 用户与当前用户的相似度列表
            for (Entry<String, Integer> simObjEn : simUserEn.getValue().entrySet()) {   // 对于每个该用户
